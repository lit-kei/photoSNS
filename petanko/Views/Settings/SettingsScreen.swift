...
enum PetankoLegalText {
    ...
    4. 友達・グループ機能
    ユーザーは、友達申請、グループ参加、グループへの友達招待などの機能を利用できます。グループでは、参加メンバーに投稿内容が共有されます。友達からグループに招待された場合、承認操作なしで参加状態になることがあります。

    5. 通報とブロック
    不適切な投稿を見つけた場合、ユーザーはアプリ内の報告機能から投稿を通報できます。また、ユーザーをブロックすることで、そのユーザーの投稿表示やフレンド申請を制限できます。ブロックは設定画面のブロックしたユーザー一覧から解除できます。

    6. 運営による対応
    運営上必要と判断した場合、違反する投稿の確認、削除、表示制限、アカウントの利用制限、その他安全確保のための対応を行うことがあります。通報された内容は、確認と対応のために保存される場合があります。

    7. アカウント削除
    ユーザーは設定画面からアカウントを削除できます。削除時には、投稿も削除するか、個人情報との紐づきを匿名化して投稿を残すかを選択できます。匿名化した場合、表示名、ユーザーID、プロフィール画像との紐づき、コメントは削除されます。

    8. 通知とプライバシー
    petankoは、投稿完了、グループ投稿、友達申請、グループ招待などに関する通知を送ることがあります。通知設定はアプリ内またはiPhoneの設定から変更できます。個人情報の取り扱いについては、プライバシーポリシーを確認してください。
    https://lit-kei.github.io/petanko/privacy.html
