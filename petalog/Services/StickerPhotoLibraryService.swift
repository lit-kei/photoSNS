...
enum StickerPhotoLibraryService {
    static func saveStickerPNG(_ data: Data) async throws {
        guard !data.isEmpty,
            ...
            throw PetalogError.message("保存できるステッカー画像がありません。")
        }
    ...
    private static func ensureAddOnlyAuthorization() async throws {
        ...
        case .notDetermined:
            ...
            guard requestedStatus == .authorized || requestedStatus == .limited else {
                throw PetalogError.message("写真への追加が許可されていません。設定アプリから写真への追加を許可してください。")
            }
        case .denied, .restricted:
            throw PetalogError.message("写真への追加が許可されていません。設定アプリから写真への追加を許可してください。")
        @unknown default:
            throw PetalogError.message("写真への追加権限を確認できませんでした。")
        }
    }
